...
export default function ProjectsSection() {
    ...
    return (
        ...
        <h2 className="text-sm font-semibold text-slate-100">Active Projects</h2>
    <button
    onClick={() => setIsOpen(true)}
    className="h-7 w-7 rounded-full flex items-center justify-center bg-blue-600 text-white text-sm hover:bg-blue-500"
        >
        +
            </button>
        </div>
    ...
    projects.map((p) => (
        <div
            key={p.id}
        className="bg-slate-900 border border-slate-800 rounded-lg px-4 py-3 text-xs flex flex-col gap-2"
        >

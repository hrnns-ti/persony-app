...
export function useNotes(courseId?: string) {
    ...
    const loadNotes = useCallback(async () => {
        try {
            setLoading(true)
            setError(null)
            const data = courseId ? await noteService.getByCourse(courseId) : await noteService.getAll()
    ...
    return { notes, loading, error, addNote, updateNote, removeNote, refresh: loadNotes }
}
